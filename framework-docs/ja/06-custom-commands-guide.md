# ワークフロースキルガイド（旧カスタムコマンドガイド）

玄人開発で使用するワークフロースキル（スラッシュコマンド）の一覧と使い方です。

> **注**: v0.4.0 以降、カスタムコマンドはスキル形式（`SKILL.md`）に統合されました。スラッシュコマンドとしての使い方は変わりません。
...
ワークフロースキルは、繰り返し行う作業を自動化・標準化するためのスラッシュコマンドです。`~/.claude/skills/` にスキルディレクトリ（`SKILL.md`）として配置することで、どのプロジェクトでも利用できます。`user-invocable: true` フィールドにより、スラッシュコマンドとして起動可能になります。
...
すべてのプロジェクトでワークフロースキルを使用するには、以下のコマンドを実行します：
