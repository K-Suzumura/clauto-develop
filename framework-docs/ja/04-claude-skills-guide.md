...
```
[ユーザー] code-builder として認証機能を実装して

→ code-builder サブエージェントが起動
  → coding-standards スキルが自動適用（コーディング規約）
  → security-baseline スキルが自動適用（認証のセキュリティ）
  → test-author スキルが自動適用（テスト作成基準）
```

このように、サブエージェントは特定の役割（レビュアー、ビルダーなど）として振る舞い、その役割に必要なスキルが自動的に適用されることで、一貫した品質の成果物を得ることができます。
